...
- **Card grid / thumb** — `repeat(auto-fill, minmax(150px,1fr))`, 12px gap, 5:7 placeholder, 1px divider border, card name overlaid on the bottom of the art (absolutely positioned, semi-opaque ink background) rather than a separate strip below it, quantity badge (ink chip, white text; accent chip if foil). Reused by Collection, Decks (expanded), Missing, Meta, For Sale. (Widened from the original `minmax(76px,1fr)`/6px-gap/separate-name-strip spec during live feedback on commit `43c219d`.)
- **In-deck proportion bar (new visual, existing data)** — 3px bar above the overlaid name: grey track (`--color-neutral-300`), red fill (`--color-accent`) sized to `copies_in_deck / copies_owned`, plus a 9px accent-700 `"{in_deck}/{owned} in deck"` label. Replaces the old single-color glow ring so partial vs. full allocation are visually distinct. Computed from data already available client-side (`allocatedCards` vs. the collection map) — rendering only, no new data plumbing.
